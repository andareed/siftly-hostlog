...
func (m *model) footerView(width int) string {
	...
	st := footerState{
		...
		StatusMessage: "",
		Legend:        "(? help · f filter · / search · t time window · T reset window · > start · < end · c edit comment · v view comments)",
	}
	if m.data.filterPattern != "" {
		st.FilterLabel = m.data.filterPattern
	}
	if m.ui.noticeMsg != "" {
